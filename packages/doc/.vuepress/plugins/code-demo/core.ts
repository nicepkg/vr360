...
import {DEFAULT_EDITOR_DESCRIPTION, DEFAULT_EDITOR_TITLE, DEFAULT_VR360_VERSION} from './constant'
...
export type EditorOptions = {
  ...
  /**
   * `true` 如果 `ion-app` 和 `ion-content` 应该自动注入到
   * Stackblitz 示例。
   */
...
const openHtmlEditor = async (code: string, options?: EditorOptions) => {
  ...
  sdk.openProject({
    ...
    dependencies: {
      '@nicepkg/vr360-core': DEFAULT_VR360_VERSION
    }
  })
}
...
const openAngularEditor = async (code: string, options?: EditorOptions) => {
  ...
  let final_app_module_ts = app_module_ts

  if (options?.angularModuleOptions) {
    if (options.angularModuleOptions.imports) {
      final_app_module_ts = `${options.angularModuleOptions.imports.join('\n')}\n${app_module_ts}`
    }
    if (options.angularModuleOptions.declarations) {
      final_app_module_ts = final_app_module_ts.replace(
        '/* CUSTOM_DECLARATIONS */',
        options.angularModuleOptions.declarations.map(d => `\n  ${d}`).join(',')
      )
    }
  }
  ...
  sdk.openProject({
    ...
    files: {
      ...
      'src/app/app.module.ts': final_app_module_ts,
    ...
    dependencies: {
      '@ionic/angular': DEFAULT_VR360_VERSION,
      /**
       ...
       */
      '@ionic/core': DEFAULT_VR360_VERSION
    }
  })
}
